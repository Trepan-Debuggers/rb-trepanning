...
(setq tb (gethash "traceback" rbdbgr-dbgr-pat-hash))

(defun tb-loc-match(text) 
  (string-match (rbdbg-dbgr-loc-pat-regexp tb) text)
)
...
(context "traceback location matching: "
	 ...
	 (lexical-let ((text "	from /usr/local/bin/irb:12:in `<main>'"))
	   ...
	   (specify "extract file name"
	   	    (expect (match-string (rbdbg-dbgr-loc-pat-file-group tb)
	   				  text)
	   		    equal "/usr/local/bin/irb")))
	   (specify "extract line number"
	   	    (expect (match-string (rbdbg-dbgr-loc-pat-line-group tb)
	   				  text)
	   		    equal "12"))
	   )
